...
<section className="text-gray-600 body-font overflow-hidden">
  <div className="container px-5 py-10 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" src={response.image} />
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        ...
        <p className="leading-relaxed">{response.description}</p>
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          {
            response.color && 
            <div className="flex">
            <span className="mr-3">Color </span>
            <button style={{background:response.color.toLowerCase()}} className={`border-2 border-gray-300  rounded-full w-6 h-6 focus:outline-none`}/>
            {/* <button className="border-2 border-gray-300 ml-1 bg-gray-700 rounded-full w-6 h-6 focus:outline-none" /> */}
            {/* <button className="border-2 border-gray-300 ml-1 bg-orange-500 rounded-full w-6 h-6 focus:outline-none" /> */}
          </div>
          }
          {
            response.size && 
        ...
        <div className="flex">
          <span className="title-font font-medium text-2xl text-gray-900"><BiRupee className='inline' /> {response.price}</span>
          <button onClick={()=>{addToCart(item, 1, response.price, response.name, response.image),  toggleCart(cartRef)}} className="flex ml-auto text-white bg-orange-400 border-0 py-2 px-6 focus:outline-none hover:bg-orange-500 rounded">Add to cart</button>
...
export async function getServerSideProps(context){
  ...
  let response = await Product.findOne({productCode: item})
  return {
    props:{
